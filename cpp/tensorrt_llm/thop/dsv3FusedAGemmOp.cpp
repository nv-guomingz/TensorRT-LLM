...
struct LoopUnroller
{
        ...
        cudaStream_t stream)
    {
        ...
        {
            tk::dsv3MinLatencyKernels::invokefusedAGemm<__nv_bfloat16, kBegin, kHdIn, kHdOut>(
...
struct LoopUnroller<kEnd, kEnd, kHdIn, kHdOut>
{
        ...
        cudaStream_t stream)
    {
        if (num_tokens == kEnd)
        {
            tk::dsv3MinLatencyKernels::invokefusedAGemm<__nv_bfloat16, kEnd, kHdIn, kHdOut>(
                output, input, weights, stream);
        }
        else
        {
    ...
    std::optional<c10::ScalarType> const& out_dtype)
{
    ...
    if (mat_a.size(0) >= 1 && mat_a.size(0) <= 16)
    {
