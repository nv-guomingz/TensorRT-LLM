...
class MambaPolicy:
    ...
    def _build_layer_ptrs(
        ...
        slot: int,
        layer_base_addresses: Optional[Dict[int, int]] = None,
        ...
        for glid in overlapping_layers:
            if layer_base_addresses is not None:
                ptrs.append(
                    layer_base_addresses[glid] + slot * (slot_stride_bytes or pool.slot_bytes)
                )
            else:
    ...
    def build_mamba_frags(
        ...
        for self_pool, peer_pool, is_conv in [
            ...
            self_layer_base_addresses = (
                self_mlg.conv_layer_base_addresses if is_conv else self_mlg.ssm_layer_base_addresses
            )
            peer_layer_base_addresses = (
                peer_mlg.conv_layer_base_addresses if is_conv else peer_mlg.ssm_layer_base_addresses
            )
            self_slot_stride_bytes = (
                self_mlg.conv_slot_stride_bytes if is_conv else self_mlg.ssm_slot_stride_bytes
            )
            peer_slot_stride_bytes = (
                peer_mlg.conv_slot_stride_bytes if is_conv else peer_mlg.ssm_slot_stride_bytes
            )
